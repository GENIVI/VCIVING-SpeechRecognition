...
class MusicPlayExecutor(TaskExecutor):
    ...
    def _play_audio_from_file(self, audio_file):
        with audio_file:
            ...
            self._play_audio_from_stream(audio_data, num_channels, self.PLAYER_BYTES_PER_SAMPLE,
                                         self.PLAYER_SAMPLING_RATE)
    ...
    def run(self, args):
        ...
        ivi_outs_mechanism_carrier_default: OutputMechanismCarrier = ivi_outs_mechanisms_carriers[
            keywords_task_executor.ARG_OUTS_MECHANISMS_MECHANISM_DEFAULT]
        ...
        if self._current_audio is None or not self._current_audio.is_playing():
            ivi_outs_mechanism_default.write_data("We are searching for the song. Please wait.",
            ...
            if song_file is not None:
                ivi_outs_mechanism_default.write_data("We found the song you've requested. Let's listen.",
                                                      wait_until_completed=True)
                self._play_audio_from_file(song_file)
            else:
                ivi_outs_mechanism_default.write_data("We didn't find the song you've requested.",
                                                      wait_until_completed=True)
